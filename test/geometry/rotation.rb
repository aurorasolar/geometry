...
describe Geometry::Rotation do
  ...
  describe "when constructed" do
    ...
    it "must accept an X axis" do
      ...
      _(rotation).must_be_instance_of(RotationAngle)
      _(rotation.angle).must_equal 0
      _(rotation.x).must_equal Point[1,0]
      _(rotation.y).must_equal Point[0,1]
    end
  end

  it "must accept x and y axes" do
    rotation = Geometry::Rotation.new :x => [1,2,3], :y => [4,5,6]
    _(rotation.x).must_equal [1,2,3]
    _(rotation.y).must_equal [4,5,6]
  end
  ...
  it "must accept 3-element x and y axes and a dimensionality of 3" do
    ...
    _(rotation.dimensions).must_equal 3
  end

  it "must reject 3-element x and y axes and a dimensionality of 2" do
    _(lambda { Geometry::Rotation.new(:dimensions => 2, :x => [1,2,3], :y => [4,5,6]) }).must_raise ArgumentError
  end

  it "must promote 2-element Vectors to dimensionality of 3" do
    rotation = Geometry::Rotation.new(:dimensions => 3, :x => [1,2], :y => [4,5])
    _(rotation.dimensions).must_equal 3
    _(rotation.x).must_equal [1,2,0]
    _(rotation.y).must_equal [4,5,0]
  end

  it "must be the identity rotation if no axes are given" do
    _(Geometry::Rotation.new.identity?).must_equal true
    _(Geometry::Rotation.new(:dimensions => 3).identity?).must_equal true
  end

  it "must have a matrix accessor" do
    r = Geometry::Rotation.new(:x => [1,0,0], :y => [0,1,0])
    _(r.matrix).must_equal Matrix[[1,0,0],[0,1,0],[0,0,1]]
  end

  describe "when comparing" do
    it "must equate equal objects" do
      _(Rotation.new(x:[1,2,3], y:[4,5,6])).must_equal Rotation.new(x:[1,2,3], y:[4,5,6])
    end
  end

  describe "comparison" do
    it "must equate equal angles" do
      _(Rotation.new(angle:45)).must_equal Rotation.new(angle:45)
    end

    it "must not equate unequal angles" do
      _(Rotation.new(angle:10)).wont_equal Rotation.new(angle:45)
    end
  end

  describe "composition" do
    it "must add angles" do
      _(Rotation.new(angle:45) + Rotation.new(angle:45)).must_equal Rotation.new(angle:90)
    end

    it "must subtract angles" do
      _(Rotation.new(angle:45) - Rotation.new(angle:45)).must_equal Rotation.new(angle:0)
    end
  ...
  describe "when transforming a Point" do
    describe "when no rotation is set" do
      it "must return the Point" do
        _(Rotation.new.transform(Point[1,0])).must_equal Point[1,0]
      end
    end
