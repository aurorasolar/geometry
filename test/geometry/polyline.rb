...
describe Geometry::Polyline do
  ...
  it "must create a Polyline object with no arguments" do
    ...
    _(polyline).must_be_kind_of Polyline
    _(polyline.edges.count).must_equal 0
    _(polyline.vertices.count).must_equal 0
  end

  it "must create a Polyline object from array arguments" do
    polyline = Geometry::Polyline.new([0,0], [1,0], [1,1], [0,1])
    _(polyline).must_be_kind_of Polyline
    _(polyline.edges.count).must_equal 3
    _(polyline.vertices.count).must_equal 4
  end
  ...
  describe "when the Polyline is closed" do
    ...
    it "must be closed" do
      _(closed_unit_square.closed?).must_equal true
    end
    ...
    it "must clone and reverse" do
      ...
      _(reversed.vertices).must_equal vertices.reverse
      _(reversed).wont_be_same_as subject
      _(reversed.closed?).must_equal true
    end

    it "must reverse itself" do
      original = subject.vertices.dup
      subject.reverse!
      _(subject.vertices.to_a).must_equal original.reverse
      _(subject.closed?).must_equal true
    end

    it "must generate bisectors" do
      _(closed_unit_square.bisectors).must_equal [Vector[1, 1], Vector[-1, 1], Vector[-1, -1], Vector[1, -1]]
    end

    it "must generate bisectors with an inside corner" do
      _(closed_concave_polyline.bisectors).must_equal [Vector[1,1], Vector[-1,-1], Vector[1,1], Vector[-1,1], Vector[-1,-1], Vector[1,-1]]
    end

    it "must generate left bisectors" do
      _(closed_unit_square.left_bisectors).must_equal [Vector[1, 1], Vector[-1, 1], Vector[-1, -1], Vector[1, -1]]
    end

    it "must generate left bisectors with an inside corner" do
      _(closed_concave_polyline.left_bisectors).must_equal [Vector[1,1], Vector[1,1], Vector[1,1], Vector[-1,1], Vector[-1,-1], Vector[1,-1]]
    end

    it "must generate right bisectors" do
      _(closed_unit_square.right_bisectors).must_equal [Vector[-1,-1], Vector[1,-1], Vector[1,1], Vector[-1,1]]
    end

    it "must generate right bisectors with an inside corner" do
      _(closed_concave_polyline.right_bisectors).must_equal [Vector[-1,-1], Vector[-1,-1], Vector[-1,-1], Vector[1,-1], Vector[1,1], Vector[-1,1]]
    end

    it "must generate spokes" do
      _(closed_unit_square.spokes).must_equal [Vector[-1,-1], Vector[1,-1], Vector[1,1], Vector[-1,1]]
    end
    ...
    it "must rightset a closed concave polyline where the first outset edge intersects with the last outset edge" do
      ...
      _(polyline.offset(-1)).must_equal Polyline.new [1, 0], [3, 0], [3, 3], [-2, 3], [-2, -2], [3, -2]
    end
  end
  ...
  describe "when the Polyline is open" do
    ...
    it "must not be closed" do
      _(unit_square.closed?).must_equal false
    end
    ...
    it "must clone and reverse" do
      reversed = subject.reverse
      _(reversed.vertices).must_equal subject.vertices.reverse
      _(reversed).wont_be_same_as subject
      _(reversed.closed?).wont_equal true
    end
    ...
    it "must reverse itself" do
      ...
      subject.reverse!
      _(subject.vertices.to_a).must_equal original.reverse
      _(subject.closed?).wont_equal true
    end

    it "must generate bisectors" do
      _(unit_square.bisectors).must_equal [Vector[0, 1], Vector[-1, 1], Vector[-1, -1], Vector[0, -1]]
    end

    it "must generate bisectors with an inside corner" do
      _(concave_polyline.bisectors).must_equal [Vector[0,1], Vector[-1,-1], Vector[1,1], Vector[-1,1], Vector[-1,-1], Vector[0,-1]]
    end

    it "must generate left bisectors" do
      _(unit_square.left_bisectors).must_equal [Vector[0, 1], Vector[-1, 1], Vector[-1, -1], Vector[0, -1]]
      _(reverse_unit_square.left_bisectors).must_equal [Vector[0, 1], Vector[1, 1], Vector[1, -1], Vector[0, -1]]
    end

    it "must generate right bisectors" do
      _(unit_square.right_bisectors).must_equal [Vector[0,-1], Vector[1,-1], Vector[1,1], Vector[0,1]]
    end

    it "must generate right bisectors with an inside corner" do
      _(concave_polyline.right_bisectors).must_equal [Vector[0,-1], Vector[-1,-1], Vector[-1,-1], Vector[1,-1], Vector[1,1], Vector[0,1]]
    end

    it "must generate left bisectors with an inside corner" do
      _(concave_polyline.left_bisectors).must_equal [Vector[0,1], Vector[1,1], Vector[1,1], Vector[-1,1], Vector[-1,-1], Vector[0,-1]]
    end

    it "must generate spokes" do
      _(unit_square.spokes).must_equal [Vector[0,-1], Vector[1,-1], Vector[1,1], Vector[0,1]]
    end
  end
  ...
  describe "when creating a Polyline from an array of Points" do
    it "must ignore repeated Points" do
      ...
      _(polyline).must_be_kind_of Geometry::Polyline
      _(polyline).must_equal Geometry::Polyline.new([0,0], [1,0], [1,1], [0,1])
    end
    ...
    it "must collapse backtracking Edges" do
      ...
      _(polyline).must_equal Geometry::Polyline.new([0,0], [2,0], [2,2], [0,2]) end
  end
  ...
  describe "when offsetting" do
    describe "with a positive offset" do
      it "must leftset a unit square" do
        ...
        _(unit_square.offset(0.1)).must_equal expected_polyline
      end
      ...
      it "must leftset a simple concave polyline" do
        ...
        _(offset_polyline).must_equal Polyline.new [0,1], [3,1], [3,2], [0,2], [0,3]
      end
    end
    ...
    describe "with a negative offset" do
      ...
      it "must rightset a concave polyline with multiply-intersecting edges" do
        ...
        offset_polyline = concave_polyline.offset(-2)
        _(offset_polyline).must_equal Polyline.new [0,-2], [7,-2], [7,4], [0,4]
      end
      ...
      it "must rightset a closed concave polyline with multiply-intersecting edges" do
        ...
        offset_polyline = concave_polyline.offset(-2)
        _(offset_polyline).must_equal Polyline.new [-2,-2], [7,-2], [7,4], [-2,4]
      end
